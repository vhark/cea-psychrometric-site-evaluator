...
This is a historical research record. Software availability, service terms, vendor prices and access probes describe the inspection dates, not a fresh release-time check. Current implementation is [IMPLEMENTATION.md](IMPLEMENTATION.md); current executed evidence is [VERIFICATION.md](VERIFICATION.md). The external source findings do not validate later model outputs or revive withdrawn class/DOAS comparisons.
...
| [NASA POWER](https://power.larc.nasa.gov/docs/services/api/temporal/hourly/) | Hourly meteorology and solar for the combined use case from 2001; meteorology approximately 0.5 x 0.625 degrees and solar 1 x 1 degree. | Keyless and [open reuse with attribution](https://registry.opendata.aws/nasa-power/), but coarse spatial resolution remains substantive. The original research quoted short nominal variable latencies; actual hourly availability in this project lagged by months. Use the returned cutoff and [CLIMATES.md](CLIMATES.md), not a guaranteed 5-to-7-day solar delay. The small browser probe succeeded only at its recorded check time. |

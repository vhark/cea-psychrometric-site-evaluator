...
Status: five sites bundled, 50 complete calendar years (ten each). Tulsa 2025 was retrieved 2026-09-11 and its other nine years 2026-09-13; the four climate archetypes (Phoenix, Miami, Denver, Seattle) were retrieved 2026-09-14, all with NASA POWER Hourly API `v2.10.0`. Every profile figure below was computed from the committed files, not quoted from a climate summary. Staleness is tracked by `node scripts/check-vintages.mjs` against `data/weather/index.json`.
...
Each site-year is a single self-contained JSON file, `data/weather/<key>-<year>.json`, between 1.10 and 1.13 MB, listed for the application in `data/weather/index.json`. The 50 snapshots total 58,253,192 bytes (55.6 MB); the retained original API responses, three per site-year, add 49,341,897 bytes (47.1 MB) in 150 files. The four archetypes account for 44.5 MB of snapshots, 37.7 MB of retained responses and 40 checksum files: 82.2 MB in total. Only the snapshot of the selected year is fetched by the browser; the retained responses are provenance and are never requested at run time.
...
- `data/weather/<key>-<year>.sha256` holds one `sha256  filename` line for that snapshot, for example `24817db293dd7f2b105c0cb9072c718aaca19e63a9f3b9d294d884d8e7c27227  phoenix-2025.json`. The filename is relative, so verify from that directory: `cd data/weather && shasum -a 256 -c phoenix-2025.sha256`. All 51 sidecars were verified on 2026-09-14, all `OK`.
- Inside each snapshot, `raw[]` carries, per API request, the request URL, the retrieval timestamp, the SHA-256 of the original response bytes and the retained filename under `data/weather/nasa-power-*.json`. The recorded SHA-256 verifies against those retained bytes; it was checked for all 150 files on 2026-09-14, all matching. POWER's header and version block are in `sourceVersions`.
...
Coverage was verified file by file on 2026-09-14: for all 50 site-years, `coverage.expectedHours` equals the local calendar length (8,760, or 8,784 in the leap years 2016, 2020 and 2024), `coverage.weatherHours` (hours with temperature, relative humidity and pressure all present) equals it, `coverage.solarHours` equals it, the length of `hours[]` equals it, and each file's SHA-256 matches its `.sha256` file. **No gaps, no missing sentinels and no partial years in any bundled file.**
...
That is 438,360 bundled hours in total, every one of them with temperature, humidity, pressure and solar present.
...
- **Period:** all ten bundled years, 2016 to 2025, for every site, so the five sites are directly comparable over an identical window. 87,672 hours per site, 438,360 in total.
...
- **Humidity input:** POWER `RH2M`, whose range across all 438,360 hours is 0.0135 to 1.0000. No value required clamping, so no hour's humidity was altered.
- **Counts:** strict inequalities, `tempC > 30` and `tempC < 0`, counted per calendar year; the table gives the ten-year mean with the lowest and highest single years in brackets, since the range is the point.

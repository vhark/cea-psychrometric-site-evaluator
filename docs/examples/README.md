...
Load matching weather, then **Import JSON / CSV** and select a set. Export anything you need to keep first because import replaces workspace scenarios. Check site coordinates, ZIP and IANA time zone as well as the weather, especially when re-homing a Fairbanks or climate-archetype input. An imported result claim is never trusted; run again.

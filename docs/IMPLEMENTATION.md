...
`weather.js`: `fetchWeather({latitude,longitude,timezone,startDate,endDate,provider='nasa',station,signal,onProgress})`; `normalizeWeather(input)`; `loadBundledIndex()`; `loadBundledYear(siteKey,year)`; `separationKm(a,b)`; `fetchObserved(...)`. `station` has no default and `provider='iem'` rejects a missing one. `fetchWeather` returns snapshot; provider='iem' returns observation+separately labeled NASA solar.

...
function renderHeadlines(r) {
  ...
  if (state.weatherOnly) {
    ...
    target.append(metric('Hours the weather could be read', units(valid, 'h', 0), `${format(r.hours.length - valid)} h missing or unreadable`, 'eligibleHour'), metric('Pad-effective window', units(pad, 'h', 0), r.scenario.padEnabled===false ? `${format(weather?.opportunity?.padCoolingHours)} h evaporative cooling opportunity — pad required; installed availability 0 h` : 'Installed pad weather availability, not equipment runtime', 'padEffectiveness'), metric('Hottest outdoor hour', units(weather?.extremes?.maxTempC, '°C', 1), 'Outside air only. This is not a prediction of the room', 'celsius'), metric('Weather coverage', r.hours.length ? `${format(valid / r.hours.length * 100, 1)}%` : 'Not available', `${format(valid)} of ${format(r.hours.length)} expected hours`));
  } else {

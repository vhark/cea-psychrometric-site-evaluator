...
export const DATASETS = [
  ...
  {
    id: 'zip-timezones',
    label: 'ZIP to IANA time zone table',
    use: 'Proposing the site time zone from a ZIP, which every local day, photoperiod and daily metric is built on.',
    manifest: 'data/us-zip-timezones.json',
    field: 'source.retrievedAt',
    vintageMeans: 'retrieval date of the GeoNames gazetteer the table was built from',
    agingAfterMonths: 18,
    staleAfterMonths: 36,
    rationale: 'Time zone boundaries move rarely, and when they do it is a county or a tribal nation at a time rather than a redraw. New ZIPs appear faster than boundaries change, and a ZIP the table lacks falls back to a labelled state guess rather than failing, so a year and a half is tolerable. Past three years enough new ZIPs are missing that the fallback stops being an edge case.',
    refreshCommand: 'node scripts/build-zip-timezones.mjs <unpacked GeoNames US.txt>',
    note: 'Rebuild after refreshing the ZIP inventory, so the two stay on the same set of ZIPs. The hand-checked override list in the build script is the place to correct a ZIP the nearest-feature lookup puts on the wrong side of a line; each entry records why. Scored 99.5% correct against tz-database boundaries when built.',
    remote: null,
  },
  {
    id: 'study-weather-years',
    label: 'Weather years the committed studies were computed on',
    use: 'The climate profiles, the regional study and the sensitivity screen.',
    manifest: 'data/weather/index.json',
    field: 'sites.*.studyYears.*',
    vintageMeans: 'newest calendar year of hourly weather any committed study used',
    agingAfterMonths: 15,
    staleAfterMonths: 24,
    rationale: 'POWER hourly data lag real time by two to three months, so a completed calendar year becomes fetchable around March. Fifteen months after the newest study year ends a newer complete year exists and no published figure has seen it; at 24 months the studies are two years behind and "recent climate" stops being true.',
    refreshCommand: 'python3 scripts/fetch-weather.py --site <key> <year>',
    note: 'The hourly records themselves are not committed: this tool retrieves weather per ZIP at run time. Refreshing a study means fetching its years again, rerunning scripts/regional-study.mjs or scripts/morris-screening.mjs, and updating sites[].studyYears in data/weather/index.json. index.json records no source URL, so --check-remote cannot probe upstream for this dataset.',
    remote: null,
  },
  {

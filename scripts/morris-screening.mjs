...
// Tulsa is fixed here because docs/SENSITIVITY.md and docs/morris-screening.json are published against it;
// re-siting the screen invalidates those figures rather than moving them.
...
function loadYear(year){
 ...
 const file=`tulsa-${year}.json`;
 if(!existsSync(here(`../data/weather/${file}`)))throw new Error(
  `No hourly weather for tulsa ${year}. The hourly weather archive is not committed: this tool retrieves weather per ZIP at run time, and only the study findings are kept. Fetch the years again before regenerating:\n  python3 scripts/fetch-weather.py --site tulsa ${site?.studyYears?.join(" ")??year}\nthen rerun this script. The fetcher writes data/weather/tulsa-<year>.json.`);

...
function loadYear(site,year){
  const file=`${site.key}-${year}.json`;
  if(!existsSync(here(`../data/weather/${file}`)))throw new Error(
    `No hourly weather for ${site.key} ${year}. The hourly weather archive is not committed: this tool retrieves weather per ZIP at run time, and only the study findings are kept. Fetch the years again before regenerating:\n  python3 scripts/fetch-weather.py --site ${site.key} ${site.studyYears?.join(" ")??year}\nthen rerun this script. The fetcher writes data/weather/${site.key}-<year>.json.`);
...
async function main(){
  ...
  for(const site of sites){
    const missing=options.years.filter(year=>!existsSync(here(`../data/weather/${site.key}-${year}.json`)));
    if(missing.length)throw new Error(
      `${site.label} has no hourly weather for ${missing.join(', ')}. The hourly archive is not committed: `
      +`this tool retrieves weather per ZIP at run time and only the study findings are kept. Fetch them first:\n`
      +`  python3 scripts/fetch-weather.py --site ${site.key} ${missing.join(' ')}`);
  }

...
Run: python3 scripts/fetch-weather.py --site KEY [year ...]   (--site is required; default year 2025)
...
SITES = {
    'tulsa': dict(key='tulsa', label='Tulsa, OK', zip='74103', latitude=36.15, longitude=-95.99,
                  timezone='America/Chicago', climate='humid subtropical',
                  rawPrefix='', notes=''),
...
def main(site, year=DEFAULT_YEAR):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--site', required=True, choices=list(SITES),
                        help='bundled site key; required, because no site is the default one')
    ...
    for requested in (arguments.years or [DEFAULT_YEAR]):
        try:
            if not main(arguments.site, year=requested):

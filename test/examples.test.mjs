...
// One committed week per site. The full hourly archive is not shipped, so these fixtures are what
// keeps every shipped example exercised against a real record rather than a synthetic one.
const weekOf = site => JSON.parse(readFileSync(`test/fixtures/${site}-2025-week.json`, 'utf8'));
const bundled = JSON.parse(readFileSync('data/weather/index.json', 'utf8')).sites;
// An example is only meaningful against its own climate: equipment sized for one site is not sized
// for another, and a single shared week would let a mis-homed set pass on weather it never sees.
const near = (a, b) => Math.abs(a - b) < 1e-6;
const weeks = new Map();
const weekFor = scenario => {
  const site = bundled.find(s => near(s.latitude, scenario.latitude) && near(s.longitude, scenario.longitude));
  assert.ok(site, `no bundled site at ${scenario.latitude},${scenario.longitude} for "${scenario.name}"; a shipped example must run offline`);
  if (!weeks.has(site.key)) weeks.set(site.key, weekOf(site.key));
  return weeks.get(site.key);
};
...
// The climate set is the one place where the same unchanged facility must meet all six records, so it is
// checked site by site even though the test above already runs every example on its own weather.
test('every climate-archetype scenario simulates its own site record', () => {
  const set = load('climate-archetypes.json');
  const byLocation = new Map(set.scenarios.map(s => [`${s.latitude},${s.longitude}`, s]));
  for (const site of bundled) {
    const scenario = byLocation.get(`${site.latitude},${site.longitude}`);
    const record = weekOf(site.key);

...
const manifests=(overrides={})=>({
 ...
 'data/weather/index.json':{sites:[{key:'tulsa',studyYears:[2016,2024,2025]}]},
 'data/us-zip-timezones.json':{source:{retrievedAt:'2026-09-11'}},
...
test('each dataset is judged against its own cadence, so equal ages give different statuses',()=>{
 ...
 assert.deepEqual([...current.values()].map(item=>[item.id,item.vintage,item.ageMonths,item.status]),[
  ...
  ['zip-timezones','2026-09-11',0,'current'],
  ['study-weather-years','2025',8,'current'],
 ...
 assert.equal(assess({},2026,9,14).get('study-weather-years').vintage,'2025');
});
test('an unreadable vintage is unknown, never current, and never invents a date',()=>{
 const missing=assess({'data/energy/coverage.json':null,'data/reference/noaa-provenance.json':{},
  ...
  'data/weather/index.json':{sites:[{studyYears:[]}]},
  'data/us-zip-timezones.json':{source:{retrievedAt:'not a date'}}},2026,9,14);
 for(const id of ['eia861m-prices','oedi-utility-mapping','egrid-grid-mix','epa-zip-subregions','geonames-zip-inventory','zip-timezones','study-weather-years','noaa-reference-daily']){
 ...
 assert.equal(summary.counts.unknown,8);
 assert.equal(summary.unknown.length,8);
...
test('the gate exits nonzero only when a dataset is past its own budget',()=>{
 ...
 assert.match(stale.headline,/8 stale/);
 assert.equal(stale.counts.stale,8);
 ...
 assert.deepEqual(fresh.counts,{current:8,aging:0,stale:0,unknown:0});
 assert.equal(fresh.worst,'current');
 assert.equal(fresh.exitCode,0);
 assert.equal(fresh.headline,'8 current, 0 aging, 0 stale, 0 unknown');

...
import {DEFAULT_SCENARIO,FACILITIES,FACILITY_TEMPLATES,FT2_PER_M2,OPAQUE_FACILITIES,SCENARIO_SCHEMA_VERSION,SYSTEMS,airflowEvidenceWarnings,applyTechnology,fixtureWm2,makeScenario,migrateScenario,validateScenario} from '../src/config.js';
...
const diurnal=(hours=48,mean=18,amp=6)=>({schemaVersion:1,source:'Synthetic diurnal fixture',sourceKind:'test',latitude:36.15,longitude:-95.99,latitude:40,longitude:-100,timezone:'UTC',startDate:'2025-04-01',endDate:'2025-04-02',
...
const weather=(hours=24,changes={})=>({schemaVersion:1,source:'Synthetic boundary-test fixture',sourceKind:'test',latitude:36.15,longitude:-95.99,latitude:40,longitude:-100,timezone:'UTC',startDate:'2025-01-01',endDate:'2025-01-01',hours:Array.from({length:hours},(_,i)=>({time:Date.UTC(2025,0,1,i),tempC:22,rh:.6,pressurePa:101325,ghiWm2:0,...changes}))});
// A scenario ships with no site, so every fixture states one. Round neutral coordinates, not a city.
const SITE={latitude:40,longitude:-100,timezone:'UTC'};
const closed=(overrides={})=>({...makeScenario('indoor'),outsideAirReviewed:true,latitude:40,longitude:-100,timezone:'UTC',areaM2:100,canopyM2:100,heightM:4,dayTargetC:22,nightTargetC:22,tempToleranceC:2,vpdMin:.5,vpdMax:1.5,maxDewPointC:19,uValue:.05,infiltrationACH:0,minVentACH:0,maxVentACH:0,lightWm2:0,dliTarget:0,transpirationModel:'schedule',transpirationLDayM2:0,cropSensibleWm2:0,coolingKW:0,dehuKgH:0,heaterKW:0,humidifierKgH:0,...overrides});
...
test('airflow evidence identifies source scope and height-dependent controlled-air context',()=>{
 const s={...makeScenario('greenhouseGlass'),...SITE,heightM:4,maxVentACH:20};
...
test('mushroom controlled outdoor air stays empty until a project design is reviewed',()=>{
 const mushroom={...makeScenario('greenhouseDouble','mushroom','mushroom'),...SITE};
...
test('high-light fixture presets convert their declared W/ft² footprints to W/m²',()=>{
 assert.ok(Math.abs(fixtureWm2('led650')/FT2_PER_M2-40.625)<.01,`LED 650 W over 16 ft² must read 40.625 W/ft², got ${fixtureWm2('led650')/FT2_PER_M2}`);
 assert.ok(Math.abs(fixtureWm2('hps1000')/FT2_PER_M2-62.5)<.01,`HPS 1000 W over 16 ft² must read 62.5 W/ft², got ${fixtureWm2('hps1000')/FT2_PER_M2}`);
 assert.equal(fixtureWm2('custom'),null,'the custom preset carries no fixture');
 const hps=closed({lightWm2:Math.round(fixtureWm2('hps1000')*10)/10,efficacy:1.9});
 assert.ok(!validateScenario(hps).some(error=>error.startsWith('Installed fixture power')),'the HPS preset density must fit the validated range');
});
test('pad runtime is counted only when installed and only where pad leaving air helps',()=>{
 ...
 const base={...makeScenario('greenhouse'),outsideAirReviewed:true,latitude:40,longitude:-100,timezone:'UTC',dayTargetC:24,nightTargetC:24,tempToleranceC:2,coolingKW:0,dehuKgH:0,heaterKW:0};
...
test('outdoor air is a dehumidifier only when drier than the zone ceiling, with a heating penalty when cold',()=>{
 const s={...makeScenario('greenhouse'),outsideAirReviewed:true,latitude:40,longitude:-100,timezone:'UTC',dehuLPerKWh:2.5};
...
test('stacked greenhouse crop light cannot create photons beyond roof input',()=>{
 const s={...makeScenario(),outsideAirReviewed:true,latitude:40,longitude:-100,timezone:'UTC',areaM2:100,canopyM2:400,parTransmission:1,shadeFraction:0,lightWm2:0,transpirationLDayM2:0};
...
test('staged controller results converge with control cadence on the Tulsa example strategies',()=>{
 const scenarios=JSON.parse(readFileSync(new URL('../docs/example-scenarios.json',import.meta.url),'utf8')).scenarios.map(raw=>{
  const s=migrateScenario(raw);s.outsideAirReviewed=true;assert.deepEqual(validateScenario(s),[]);return {...s,latitude:40,longitude:-100,timezone:'UTC'};
 });
 const w=diurnal();
...
test('state-coupled crop moisture responds to zone humidity where a schedule cannot',()=>{
 const s={...makeScenario('greenhouse'),outsideAirReviewed:true,latitude:40,longitude:-100,timezone:'UTC',dehuKgH:0};
...
test('older scenario JSON without later keys migrates with crop-specific inert defaults',()=>{
 const legacy={...makeScenario('greenhouseDouble','bench','mushroom'),...SITE,schemaVersion:1};
 ...
 const lettuce={...makeScenario('greenhouseDouble'),...SITE};delete lettuce.lai;delete lettuce.transpirationModel;
...
test('supported cultivation systems validate when required project airflow is complete; a retired one fails by name',()=>{
 for(const system of Object.keys(SYSTEMS)){
  const s={...makeScenario('greenhouseDouble',system),...SITE};
 ...
 const errors=validateScenario({...makeScenario('greenhouseDouble'),...SITE,system:'wall'});
...
test('a schemaVersion 1 non-DOAS scenario simulates without a prior validate call',()=>{
 const legacy={...makeScenario('greenhouseDouble'),...SITE,schemaVersion:1};
 for(const key of ['controlMode','transpirationModel','lai','doasM3s','doasSupplyDewPointC','doasSupplyTempC'])delete legacy[key];
...
test('scenario JSON written before the component keys existed validates and simulates unchanged',()=>{
 const legacy={...makeScenario('greenhouseDouble'),...SITE};
...
test('every facility template carries a sourced envelope and explicit airflow review status',()=>{
 for(const key of Object.keys(FACILITIES)){
  ...
  const candidate={...s,...SITE,outsideAirReviewed:true};
...
const coldDry=(hours=48)=>({schemaVersion:1,source:'Synthetic cold-dry fixture',sourceKind:'test',latitude:64.84,longitude:-147.72,latitude:40,longitude:-100,timezone:'UTC',startDate:'2025-01-01',endDate:'2025-01-02',
 hours:Array.from({length:hours},(_,i)=>({time:Date.UTC(2025,0,1,i),tempC:-22,rh:.7,pressurePa:99000,ghiWm2:0}))});
const litBox=(over={})=>({...makeScenario('warehouseSip','bench','lettuce'),outsideAirReviewed:true,latitude:40,longitude:-100,timezone:'UTC',areaM2:500,heightM:4,
...
test('a dehumidifier releases its heat where the topology says, and the split conserves',()=>{
  const wet=(hours=48)=>({schemaVersion:1,source:'Synthetic warm-wet fixture',sourceKind:'test',latitude:25.77,longitude:-80.19,latitude:40,longitude:-100,timezone:'UTC',startDate:'2025-07-01',endDate:'2025-07-02',
   hours:Array.from({length:hours},(_,i)=>({time:Date.UTC(2025,6,1,i),tempC:28,rh:.85,pressurePa:101325,ghiWm2:0}))});
  const room=over=>({...makeScenario('indoor','bench','lettuce'),outsideAirReviewed:true,latitude:40,longitude:-100,timezone:'UTC',areaM2:200,heightM:4,
